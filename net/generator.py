# import tensorflow as tf_contrib
import tensorflow as tf
import tf_slim as slim


def layer_norm(x, scope='layer_norm'):
    return slim.layers.layer_norm(x,
...
def dwise_conv(input, k_h=3, k_w=3, channel_multiplier=1, strides=[1, 1, 1, 1],
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        w = tf.compat.v1.get_variable('w', [k_h, k_w, in_channel, channel_multiplier], regularizer=None,
                            initializer=slim.variance_scaling_initializer())
        conv = tf.nn.depthwise_conv2d(input, w, strides, padding, name=name, data_format=None)
        if bias:
            biases = tf.compat.v1.get_variable('bias', [in_channel * channel_multiplier],
